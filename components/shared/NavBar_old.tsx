...
export function Navbar({
  ...
  return (
    <>
      <nav
        className={`${home ? "" : "fixed"} ${bgHover ? "hover:bg-tersier" : ""
          } z-[200] top-0 px-5 w-full ${scrollPosition?.y ?? 0 >= scrollP
            ? home
              ? ""
              : `bg-tersier shadow-tersier shadow-sm ${shrink ? "py-1" : `${paddingY}`
              }`
            : `${paddingY}`
          } transition-all duration-200 ease-linear`}
      >
        <div
          className={`flex items-center justify-between mx-auto ${home ? "lg:max-w-[90%] gap-10" : "max-w-screen-2xl"
            }`}
        >
          <div
            className={`flex items-center ${withNav ? `${home ? "" : "w-[20%]"} gap-8` : " w-full gap-4"
              }`}
          >
            {info ? (
              <>
              ...
              <Link
                href={"/en"}
                className={`flex-center font-outfit font-semibold pb-2 ${home ? "text-4xl text-action" : "text-white text-3xl"
                  }`}
              >
                moopa
              </Link>
          ...
          {withNav && (
            <ul
              className={`hidden w-full items-center gap-10 pt-2 font-outfit text-[14px] lg:pt-0 lg:flex ${home ? "justify-start" : "justify-center"
                }`}
            >
              <li>
                <Link
